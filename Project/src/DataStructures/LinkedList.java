...
public class LinkedList<T> {
	...
	public LinkedListNode<T> getLastNode() {
		...
		if (isEmpty()) {
			...
			System.err
					.println("getLast was called and there aren't any nodes!");
		}
		// Otherwise
		else {
	...
	public void deleteLast() {
		...
		while (node.getNext().getNext() != null)
			// Get the next node
			node = node.getNext();
		// Set the next node to null, making the current node (one up from the
		// tail) the new tail
		node.setNext(null);
	...
	public void deleteNext(LinkedListNode<T> currentNode) {
		// Set the currentNode to point to the one after the next node. This
		// will drop the one in the middle from the list
